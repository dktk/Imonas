...
namespace Infrastructure.Services.Identity
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<(Result<IdentityResult> Result, string UserId)> CreateUserAsync(string userName, string password)
        {
        ...
        public async Task<Result<TokenResponseDto>> LoginAsync(TokenRequestDto request)
        {
            ...
            if (user == null)
            {
                return Result<TokenResponseDto>.CreateFailure(new string[] { _localizer["User Not Found."] });
            }
            if (!user.IsActive)
            {
                return Result<TokenResponseDto>.CreateFailure(new string[] { _localizer["User Not Active. Please contact the administrator."] });
            }
            if (!user.EmailConfirmed)
            {
                return Result<TokenResponseDto>.CreateFailure(new string[] { _localizer["E-Mail not confirmed."] });
            }
            var passwordValid = await _userManager.CheckPasswordAsync(user, request.Password);
            if (!passwordValid)
            {
                return Result<TokenResponseDto>.CreateFailure(new string[] { _localizer["Invalid Credentials."] });
            }
            ...
            return Result<TokenResponseDto>.CreateSuccess(response);
        }
        ...
        public async Task<Result<TokenResponseDto>> RefreshTokenAsync(RefreshTokenRequestDto request)
        {
            if (request is null)
            {
                return Result<TokenResponseDto>.CreateFailure(new string[] { _localizer["Invalid Client Token."] });
            }
